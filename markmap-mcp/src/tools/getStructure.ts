...
import { sanitizeAgentId, saveStructureOutputs } from '../lib/storage.js';
...
export async function getStructureTool(
  ...
  try {
    ...
    // Auto-save markdown + JSON structure
    const saved_files = await saveStructureOutputs(
      input.agent_id,
      'getStructure',
    ...
    return {
      content: [{
        type: 'text',
        text: `Analyzed structure: ${stats.nodeCount} nodes, ${stats.maxDepth} levels deep, ${stats.totalCharacters} characters. Files saved to ${saved_files.md}`
      }],
