...
import { initStorageRoot, MARKMAP_STORAGE_ROOT } from './lib/storage.js';

/**
 * Common agent_id property definition shared by all tools
 */
const AGENT_ID_PROPERTY = {
  agent_id: {
    type: 'string',
    description: 'Unique agent identifier for isolated output storage'
  }
} as const;
...
const TOOLS: Tool[] = [
  {
    ...
    inputSchema: {
      ...
      required: ['agent_id', 'markdown_content']
    }
  },
  {
    ...
    inputSchema: {
      ...
      required: ['agent_id', 'outline_items']
    }
  },
  {
...
async function main() {
  ...
  try {
    ...
    console.error(`Storage root initialized: ${MARKMAP_STORAGE_ROOT}`);
  } catch (error) {
    console.error(`Warning: Could not create storage root at ${MARKMAP_STORAGE_ROOT}: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
