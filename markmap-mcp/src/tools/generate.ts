...
export async function generateTool(input: GenerateInput): Promise<ToolResponse<GenerateOutput> | ToolError> {
  try {
    ...
    // Auto-save outputs
    const saved_files = await saveOutputs(input.agent_id, 'generate', svg_content, input.markdown_content);
    ...
    return {
      content: [{
        type: 'text',
        text: `Successfully generated mindmap with ${nodeCount} nodes and ${depth} levels of depth. Files saved to ${saved_files.svg}`
      }],
